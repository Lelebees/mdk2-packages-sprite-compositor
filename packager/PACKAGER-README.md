...
A tool for packaging .NET Shared Projects (.shproj) as NuGet packages, enabling easy distribution and reuse of shared code libraries.
...
When you install the `Malforge.Mdk.SharedNuGet` package in your demo project, it automatically packages all referenced shared projects on Release builds.
...
- **`_targetframework`**: If not specified, defaults to `any` in direct mode, or matches the demo project's target framework in demo-driven mode
